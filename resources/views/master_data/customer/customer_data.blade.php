        ...
        <div class="card">
            <div class="card-header">
                <div class="col-12 col-md-6 col-lg-6">
                    <a href="{{route('customer.create')}}" class="btn btn-icon icon-left btn-primary"><i class="far fa-edit"></i>Tambah data</a>
                    <button id="btnDel" type="button" class="btn btn-danger ml-2 btnDel" data-url="{{ url('deleteall/customer') }}"><i class="fa fa-trash"></i> Hapus Checked</button>
                    ...
                    <tbody>
                        ...
                        <tr id="tr_{{$data->id}}">
                            <td><input type="checkbox" class="custom-checkbox cekbox" data-id="{{$data->id}}"></td>
                                ...
                                <td>{{$data->address}}</td>
                                <td class="text-right">
                                    <a href="{{route('customer.edit',$data->id)}}" class="btn btn-icon icon-left btn-primary btn-sm"><i class="far fa-edit"></i>Update</a>
                                    <a href="#" data-id="{{$data->id}}"class="btn btn-icon icon-left btn-danger btn-sm swal-confrim">
                                  
                                        <form action="{{route('customer.destroy',$data->id)}}" id="delete{{$data->id}}" method="POST">
                                            @csrf
                                            @method('delete')
                                        </form>
